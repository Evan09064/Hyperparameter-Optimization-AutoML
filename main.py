...
argparser.add_argument("--meta_batch_size", default=4, type=int, help="Number of tasks in a meta-batch")
...
argparser.add_argument("--T", default=1, type=int,
...
RDIR = f"./results/" + '-'.join(
    [str(''.join([x[0] for x in item.split('_')])) + str(value) for item, value in vars(args).items()]) + '/'
if not os.path.isdir(RDIR):
    os.mkdir(RDIR)
